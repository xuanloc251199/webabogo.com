...
use Onepay\Charge;
use Onepay\Checkout\Session as OnepayCheckoutSession;
...
class OnepayPaymentService extends OnepayPaymentAbstract
{
    public function makePayment(array $data): ?string
    {
        ...
        if ($this->isOnepayApiCharge()) {
            if (! $this->token) {
                $this->setErrorMessage(trans('plugins/payment::payment.could_not_get_onepay_token'));

                Log::error(
                    trans('plugins/payment::payment.could_not_get_onepay_token'),
                    PaymentHelper::formatLog(
                        [
                            'error' => 'missing Onepay token',
        ...
        $requestData = [
            ...
            'success_url' => route('payments.onepay.success') . '?session_id={CHECKOUT_SESSION_ID}',
            'cancel_url' => route('payments.onepay.error'),
            'metadata' => [
        ...
        $checkoutSession = OnepayCheckoutSession::create($requestData);
    ...
    public function isOnepayApiCharge(): bool
    {
        $key = 'onepay_api_charge';

        return get_payment_setting('payment_type', ONEPAY_PAYMENT_METHOD_NAME, $key) == $key;
    }
