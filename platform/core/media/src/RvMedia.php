...
class RvMedia
{
    ...
    public function url(?string $path): string
    {
        ...
        if (config('filesystems.default') === 'ftp') {
              return setting('media_ftp_custom_domain')  . '/' . ltrim($path, '/');
            return setting('media_ftp_custom_domain') . setting('media_ftp_root') . '/' . ltrim($path, '/');
        }
        return Storage::url($path);
    }

    public function setFtpDisk(array $config): void
    {
        if (
            ! $config['hostname'] ||
            ! $config['username'] ||
