...
class PublicController extends BaseController
{
    ...
    public function postEditAccount(EditAccountRequest $request)
    {
        ...
        CustomerForm::createFromModel($customer)->saving(function (CustomerForm $form) {
            $model = $form->getModel();
            $request = $form->getRequest();

            $model->fill($request->except('email'));
            $model->dob = Carbon::parse($request->input('dob'));
            $model->save();
    ...
    public function postChangePassword(UpdatePasswordRequest $request)
    {
        ...
        event(new ChangePasswordEvent($user->id));
        session()->flash('success_msg', true);
        return $this
            ->httpResponse()
    ...
    public function postUpdateSocialMedia(Request $request)
    {
        try {
            $customer = auth('customer')->user();
            ...
            return $this
                ->httpResponse()
                ->setMessage(__('Cập nhật mạng xã hội thành công!'));

        } catch (\Illuminate\Validation\ValidationException $e) {
            return $this
                ->httpResponse()
                ->setError()
                ->setMessage('Dữ liệu không hợp lệ: ' . implode(', ', $e->validator->errors()->all()));
        } catch (Exception $exception) {
            return $this
                ->httpResponse()
                ->setError()
                ->setMessage('Có lỗi xảy ra: ' . $exception->getMessage());
        }
    }
