...
class OrderHelper
{
    public function processOrder(string|array|null $orderIds, ?string $chargeId = null): bool|Collection|array|Model
    {
        ...
        foreach ($orders as $order) {
            ...
            $this->decreaseProductQuantityNew($order);
    ...
    public function decreaseProductQuantityNew(Order $order): bool
    {
        foreach ($order->products as $orderProduct) {
            $product = Product::query()->find($orderProduct->product_id);
            if ($product->type != "tour" && ($product->with_storehouse_management || $product->quantity >= $orderProduct->qty)) {
                $date = $orderProduct->options["days"];
                ...
                foreach ($availableDates as $availableProduct) {
                    if ($availableProduct->quantity > 0) {
                        $product_available = Product::query()->find($availableProduct->product_id);
                        if ($product_available) {
                            $product_available->decrement('quantity', $orderProduct->qty);
                            event(new ProductQuantityUpdatedEvent($product_available));
                        }
                    }
                }
            } else {
    ...
    public function getEmailVariables(Order $order): array
    {
        ...
        return apply_filters('ecommerce_order_email_variables', [
            ...
            'order_delivery_notes' => view(
                ...
                ->render(),
            'use_view_confirmed_order' => view(
                'plugins/ecommerce::emails.partials.order-confirmed',
                compact('order')
            )
                ->render(),
            'order' => $order->toArray(),
