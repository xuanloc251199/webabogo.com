...
//                'is_children' => 0,
    ...
    public function filterProducts(array $filters, array $params = [])
    {
        ...
        if (isset($filters['level_star'])) {
            $this->model = $this->model->where("level_star", $filters['level_star']);
        }
        ...
        $filters['places'] = array_filter($filters['places']);
        if ($filters['places']) {
            $places = $filters['places'];
            $this->model = $this->model
                ->whereHas('places', function ($q) use ($places) {
