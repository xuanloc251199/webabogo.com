...
class ProductController extends BaseController
{
    ...
    )
    {
        ...
        if ($request->has('grouped_products')) {
            $grouped = $request->input('grouped_products');

            $groupedArray = is_string($grouped)
                ? explode(',', $grouped)
                : (array)$grouped;

            GroupedProduct::createGroupedProducts(
                $product->getKey(),
                collect($groupedArray)
                    ->filter()
                    ->map(fn($id) => ['id' => $id, 'qty' => 1])
                    ->toArray()
            );
        }

        if ($request->has('blogs')) {
            $product->blogs()->sync($request->input('blogs'));
        }

        if ($request->has('extensions')) {
            $product->extensions()->sync($request->input('extensions'));
        }
    ...
    )
    {
        ...
        if ($request->has('places')) {
            $product->places()->sync($request->input('places'));
        }
        if ($request->has('extensions')) {
            $product->extensions()->sync($request->input('extensions'));
        }
