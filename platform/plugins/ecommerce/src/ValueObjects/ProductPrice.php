...
class ProductPrice
{
    ...
    public function getCurrentDatePrice($date = null): float
    {
        if (function_exists('get_current_date_price')) {
            return get_current_date_price($this->product, $date);
        }

        return $this->getPrice();
    }

    public function displayCurrentDatePriceAsText($date = null): string
    {
        $priceText = format_price($this->getCurrentDatePrice($date));

        return $this->applyFilters('current_date_price', 'display_as_text', $priceText);
    }
