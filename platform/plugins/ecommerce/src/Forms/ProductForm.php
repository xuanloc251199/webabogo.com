...
class ProductForm extends FormAbstract
{
    public function setup(): void
    {
        ...
        $blogs = Post::query()->pluck('name', 'id')->all();
        $places = Place::query()->pluck('name', 'id')->all();
        $extensions = Extensions::query()->pluck('name', 'id')->all();
        $services = Services::query()->pluck('name', 'id')->all();

        if ($this->getModel()) {
            $productId = $this->getModel()->id;

            $selectedCategories = $this->getModel()->categories()->pluck('category_id')->all();
            $selectedExtensions = $this->getModel()->extensions()->pluck('extension_id')->all();
            $selectedPlaces = $this->getModel()->places()->pluck('place_id')->all();

            $selectedBlogs = $this->getModel()->blogs()->pluck('blog_id')->all();
            $selectedServices = $this->getModel()->services()->pluck('id')->all();
        ...
        $this
            ...
            ->add('address', TextField::class, TextFieldOption::make()->label('Địa chỉ')->toArray())
            ->add(
                'description',
                EditorField::class,
            ...
            ->add('type', SelectField::class, SelectFieldOption::make()
                ...
                ->toArray())
            ->add('level_star', SelectField::class, SelectFieldOption::make()
                ->label("Chọn hạng sao")
                ->choices([
                    5 => "5 sao",
                    4 => "4 sao",
            ...
            ->add(
                ...
                MultiCheckListField::class, [
                    'label' => trans('Places'),
                    'choices' => $places,
                    'value' => old('places', $selectedPlaces),
                ]
            )
            ->add(
                'extensions[]',
                MultiCheckListField::class, [
                    'label' => trans('Tiện ích'),
                    'choices' => $extensions,
                    'value' => old('extensions', $selectedExtensions),
                ]
            )
    ...
    public function addAssets(): void
    {
        Assets::addStyles('datetimepicker')
            ...
            ->addScriptsDirectly('vendor/core/plugins/ecommerce/js/edit-product-more.js');
    }
}
