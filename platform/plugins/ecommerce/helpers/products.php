...
//Lấy ra giá nhỏ nhất cho product của các product group.
if (!function_exists('get_min_price_product_group')) {
    function get_min_price_product_group($product)
    {
        if (!$product->is_variation) {
            $groupedProduct = $product->groupedProduct()->orderBy('sale_price', "asc")->first();
            if ($groupedProduct) {
                return $groupedProduct->front_sale_price;
            }
            return $product->front_sale_price;
        }
        return get_current_date_price($product);
    }
}
