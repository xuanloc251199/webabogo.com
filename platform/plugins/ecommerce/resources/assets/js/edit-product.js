...
$(() => {
    ...
    // Tính phần trăm giảm giá khi nhập giá và giá giảm
    $(document).on('keyup', '#price, #sale_price', function () {
        let price = parseFloat($('#price').val().replace(/,/g, '')) || 0;
        let salePrice = parseFloat($('#sale_price').val().replace(/,/g, '')) || 0;
        let discountPercent = 0;

        if (price > 0 && salePrice >= 0 && salePrice < price) {
            discountPercent = 100 - Math.round((salePrice * 100) / price);
        }
        $('#discount-percent-hint').html(
            discountPercent > 0
                ? `Discount <strong>${discountPercent}%</strong> from original price.`
                : ''
        );
    });
})
