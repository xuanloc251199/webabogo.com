...
//            $isAvailableShipping = EcommerceHelper::isAvailableShipping($products);
        ...
        if (view()->exists($checkoutView)) {
            return Theme::scope("ecommerce.orders.checkout_new", $data)->render();
...
//        Cart::instance('cart')->refresh();
    ...
    public function postCheckoutCore(
        string                       $token,
    ...
    )
    {
        ...
        if (Cart::instance('cart')->isEmpty()) {
            $request->session()->flash('error_msg', __('No products in cart'));
        ...
        if (($minimumQuantity = EcommerceHelper::getMinimumOrderQuantity()) > 0
            && $totalQuality < $minimumQuantity) {
            $request->session()->flash('error_msg', __('Minimum order quantity is :qty, you need to buy more :more to place an order!', [
                'qty' => $totalQuality,
                'more' => $minimumQuantity - $totalQuality,
            ]));
            return $this
        ...
        foreach ($products as $product) {
            if ($product->isOutOfStock()) {
                ...
                $request->session()->flash('error_msg', __('Product :product is out of stock!', ['product' => $product->original_product->name]));
                return $this
                    ->httpResponse()
                    ->setError()
                    ->setMessage(
            ...
            if ($product->type != "tour") {
                $date = $product->cartItem->options["days"] ?? [];
                $availableDates = ProductVariationItem::query()
                    ...
                    ->whereIn('ec_product_attributes.title', $date)
                    ->select([
                    ...
                    ->get();
                foreach ($availableDates as $availableProduct) {
                    if ($availableProduct->with_storehouse_management == 1) {
                        if ($availableProduct->quantity < $product->cartItem->qty) {
                            $request->session()->flash('error_msg');
                            return $this
                                ->httpResponse()
                                ->setError()
                                ->setMessage(
                                    __($product->name . " đã hết phòng ngày $availableProduct->title")
                                );
                        }
                    } else {
                        if ($availableProduct->stock_status != "in_stock") {
        ...
        if (!is_plugin_active('payment') || !$orderAmount) {
            ...
            $request->session()->flash('success_msg', "Đơn đặt của bạn đã được gửi đến Abogo. Vui lòng đợi nhân viên liên hệ lại với bạn!");
//            return redirect()->to(route('public.checkout.success', OrderHelper::getOrderSessionToken()));
            return redirect()->to(route('public.cart'));
        }
        ...
        if ($checkoutUrl = Arr::get($paymentData, 'checkoutUrl')) {
            ...
            $request->session()->flash($paymentData['error'] ? 'error_msg' : 'success_msg', $paymentData['message']);
            return $this
                ->httpResponse()
                ->setError($paymentData['error'])
                ->setNextUrl($checkoutUrl)
        ...
        if ($paymentData['error'] || !$paymentData['charge_id']) {
            $request->session()->flash('error_msg', __('Checkout error!'));
...
//            ->setNextUrl(PaymentHelper::getRedirectURL($token))
            ->setNextUrl(route("public.cart"))
            ->setMessage("Đơn đặt của bạn đã được gửi đến Abogo. Vui lòng đợi nhân viên liên hệ lại với bạn!");
    }

    public function getCheckoutSuccess(string $token, Request $request)
    {
        ...
        if (is_plugin_active('payment') && (float)$order->amount && $order->payment_id && $order->payment->status == "completed") {
            event(new PaymentSuccessedEvent($order));
        }
