...
class Cart
{
    ...
    public function rawTotal()
    {
        ...
        return $content->reduce(function ($total, ?CartItem $cartItem) {
            ...
            $fee = @$cartItem->options->priceChildrenSurplus + @$cartItem->options->addOn + @$cartItem->options->price_service_other;

            if (!EcommerceHelper::isTaxEnabled()) {
                return $total + $cartItem->qty * $cartItem->price + $fee;
            }
            return $total + ($cartItem->qty * ($cartItem->priceTax == 0 ? $cartItem->price : $cartItem->priceTax)) + ($fee * $cartItem->taxRate / 100) + $fee;
        }, 0);
    ...
    public function rawSubTotal()
    {
        ...
        return $content->reduce(function ($subTotal, CartItem $cartItem) {
            return $subTotal + ($cartItem->qty * $cartItem->price) + @$cartItem->options->priceChildrenSurplus + @$cartItem->options->addOn + @$cartItem->options->price_service_other;
        }, 0);
    }
    ...
    public function rawTax()
    {
        ...
        return $content->reduce(function ($tax, CartItem $cartItem) {
            $fee = @$cartItem->options->priceChildrenSurplus + @$cartItem->options->addOn + @$cartItem->options->price_service_other;
            return $tax + ($cartItem->qty * $cartItem->tax) + $fee * $cartItem->taxRate / 100;
        }, 0);
    }
    ...
    public function subtotal()
    {
        ...
        $subTotal = $content->reduce(function ($subTotal, CartItem $cartItem) {
            $fee = @$cartItem->options->priceChildrenSurplus + @$cartItem->options->addOn + @$cartItem->options->price_service_other;
            return $subTotal + ($cartItem->qty * $cartItem->price) + $fee;
        }, 0);
