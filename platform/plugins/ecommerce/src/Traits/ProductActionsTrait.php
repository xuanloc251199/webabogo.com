...
trait ProductActionsTrait
{
    ...
    public function postAddVersion(ProductVersionRequest $request, int|string|null $id, BaseHttpResponse $response)
    {
        ...
        foreach ($addedAttributes as $setId => $attributeItem) {
            if (is_array($attributeItem)) {
                //                Lấy ra id của thuộc tính chọn khoảng ngày date-range
                $dateRangeSetId = $setId;
            } else {
                //                Lấy ra id của thuộc tính khác
                $otherAttributeIds[$setId] = $attributeItem;
            }
        }

        if ($dateRangeSetId) {
            // Có date-range: sinh nhiều biến thể theo từng ngày hợp lệ
            $dateRangeItem = $addedAttributes[$dateRangeSetId];
            $start = Carbon::parse($dateRangeItem["start_date"] ?? null);
            $end = Carbon::parse($dateRangeItem["end_date"] ?? null);
            $selectedDays = array_map('strtolower', (array) ($dateRangeItem["day-of-week"] ?? []));

            $validDates = [];
            while ($start && $end && $start <= $end) {
                $dayName = strtolower($start->format('l'));
                if (empty($selectedDays) || in_array($dayName, $selectedDays)) {
                    $validDates[] = $start->copy();
                }
                $start->addDay();
            }

            foreach ($validDates as $date) {
                $dateAttributeId = $this->createProductAttributeByDate($dateRangeSetId, $date);
                $attributeIds = [$dateAttributeId];
                foreach ($otherAttributeIds as $attrId) {
                    $attributeIds[] = $attrId;
                }
                $result = ProductVariation::getVariationByAttributesOrCreate($id, $attributeIds);
                $input = $request->input();
                $input['attribute_sets'] = $attributeIds;
                $variationData[$result['variation']->id] = $input;
            }
        } else {
            // Không có date-range: chỉ tạo 1 biến thể duy nhất
            $attributeIds = [];
            foreach ($otherAttributeIds as $attrId) {
                $attributeIds[] = $attrId;
            }
        ...
        if (!empty($variationData)) {
            $this->postSaveAllVersions($variationData, $id, $response);
    ...
    private function createProductAttributeByDate($attributeSetId, $date)
    {
    ...
    public function postUpdatePriceMultiVersions(Request $request, BaseHttpResponse $response)
    {
        $ids = (array) $request->input('ids');
        ...
        return $response
            ->setData([
                'total_product_variations' => $count,
            ])
