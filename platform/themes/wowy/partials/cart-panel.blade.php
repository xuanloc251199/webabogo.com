@if (Cart::instance('cart')->count() > 0)
                ...
                @if (!empty($product))
                    <li>
                        <div class="shopping-cart-img">
                            <a href="{{ $product->original_product->url }}"><img alt="{{ $product->name }}" src="{{ RvMedia::getImageUrl(Arr::get($cartItem->options, 'image', $product->original_product->image), 'thumb') }}"></a>
                        </div>
    ...
    <div class="shopping-cart-footer">
        ...
        <div class="shopping-cart-button">
            <a href="{{ route('public.cart') }}" class="mb-2">{{ __('View cart') }}</a>
            @if (session('tracked_start_checkout'))
                <a href="{{ route('public.checkout.information', session('tracked_start_checkout')) }}" class="mb-2">{{ __('Checkout') }}</a>
            @endif
        </div>
    </div>
@else
